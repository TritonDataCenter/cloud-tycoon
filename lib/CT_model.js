...
/*
 * This hook is run in standalone mode after the model is attached to the
 * simulator.  The main objective is to attach event handlers so that we can
 * produce sensible output in this mode.  We also attach handlers for events
 * that are fatal in standalone mode (such as suspend).  The model may
 * override this method but must be sure to cover all the fatal cases.
 */
CT_model.prototype.standalone_postattach = function (arg)
{
	var sim;
	var nop = function () {};
	...
	mod_assert.object(arg, 'arg');
	mod_assert.object(arg.simulator, 'arg.simulator');

	sim = arg.simulator;

	sim.on('ctl-response', nop);
	sim.on('ctl-error', fatal);
	...
	sim.on('suspend', function (ev) {
		...
		case CT_simulation.CTS_SUSPEND_MODEL_TRAP:
			reason = 'model-generated trap';
			break;
		case CT_simulation.CTS_SUSPEND_CTL:
			reason = 'control-stream stop command';
			break;
		/*
		 * Every simulation eventually terminates.  This is normal.
		 */
		...
		err = new VError('simulation suspended at %d (%s)',
		    sim.addr(), reason);
		fatal(err);
	});
};
...
CT_model.init_module = function (arg)
{
	...
	/*
	 * Interactive mode: set up the module and return; the interactive
	 * driver does everything else later.
	 */
	...
	/*
	 * Standalone mode: instantiate ourselves, use attach ourselves to a
	 * new simulation.  We cannot use the attach scmd here because we have
	 ...
	 * pipeline.  Once attach is complete, we run the postattach method
	 * (which will normally attach event handlers to the simulation so we
	 * can produce output) and then fire up the simulation.
	 */
	...
	sim.attach_standalone({ model: model, cb: function (_arg) {
		...
		model.standalone_postattach({ simulator: sim });
