...
CT_session.prototype.start = function (arg)
{
	...
	sim.on('ctl-error', function (_arg) { fatal(_arg.err); });
	cli.on('error', fatal);
	cs.on('error', fatal);
	rl.on('error', fatal);

	sim.on('model-data', function (_arg) {
		if (typeof (arg) !== 'undefined')
			sos.write(fmt('%s\n', JSON.stringify(_arg)));
	});

	sim.on('model-error', function (_arg) {
	...
	sim.on('suspend', function (_arg) {
		mod_assert.object(_arg, 'arg');
	...
	cs.on('parse-error', function (_arg) {
		mod_assert.object(_arg, 'arg');
