...
CT_session.prototype.start = function (arg)
{
	...
	sim.on('ctl-error', function (arg) { fatal(arg.err); });
	cli.on('error', fatal);
	cs.on('error', fatal);
	rl.on('error', fatal);

	sim.on('model-data', function (arg) {
		if (typeof (arg) !== 'undefined')
			sos.write(fmt('%s\n', JSON.stringify(arg)));
	});

	sim.on('model-error', function (arg) {
		if (typeof (arg.err) !== 'undefined') {
			ses.write(fmt('fatal simulation error: %s\n',
			    arg.err.toString()));
		}
	});

	sim.on('suspend', function (arg) {
		mod_assert.object(arg, 'arg');
	...
	cs.on('parse-error', function (arg) {
		mod_assert.object(arg, 'arg');
